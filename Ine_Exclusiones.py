...
def sql_table(con):
    ...
    cursorObj.execute("CREATE TABLE concurso(id INTEGER PRIMARY KEY, dni text, apellidos_nombre text, puesto_solic text, orden_anexo integer, cod_puesto integer, denominacion text,causa_excl text, cuerpo_concurso text)")
...
for item in listadoPrimerArreglo:
	for i in item:
		if len(i)<5 : #Debido a que hay elementos vacios se han llenado con nombre,apellidos y dni pero no hay mas datos. Lo filtramos y solo insertamos en bbdd los datos que valen
			...
			print (i)
			con.execute("INSERT INTO concurso(dni,apellidos_nombre,puesto_solic,orden_anexo,cod_puesto,denominacion,causa_excl,cuerpo_concurso) VALUES (?,?,?,?,?,?,?,?)", i) #se ponen tantas ? como elementos reales hay en el listado, no se cuenta con el id que es autoincrementado
			con.commit()